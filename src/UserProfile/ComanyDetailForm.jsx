...
const companySchema = yup.object({
  // Basic Information
  companyName: yup.string().required("Company name is required"), // Made required
  email: yup.string().email("Invalid email").required("Email is required"),
  mobile: yup.string().required("Mobile number is required"),
  landline: yup.string(),
  ...
  // Company Details
  registrationNumber: yup.string().required("Registration number is required"),
  companyType: yup.string().required("Company type is required"), // Made required
  sector: yup.string().required("Sector is required"), // Made required
  establishDate: yup.string().required("Establish date is required"), // Made required
  website: yup.string().url("Invalid website URL"),
  gstNo: yup.string(),
  panNo: yup.string(),

  // Address Information
  address1: yup.string().required("Address is required"), // Added address1
  address2: yup.string(),
  country: yup.string().default("India"),
  state: yup.string().required("State is required"), // Made required
  district: yup.string().required("District is required"), // Made required
  city: yup.string().required("City is required"), // Made required
  pinCode: yup.string().required("PIN code is required"), // Made required
...
export default function CompanyDetailForm() {
  ...
  // Form submission - UPDATED WITH REDIRECT
  const onSubmit = async (formData) => {
    try {
      const payload = {
        ...
        landline: formData.landline, // ✅ required
      ...
      const response = await fetch("https://auditfiling.com/api/v1/user/companies/store", {
        ...
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("user_name,user")}`,
        },
