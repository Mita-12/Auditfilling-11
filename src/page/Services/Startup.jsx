...
//       {/* <Header /> */}
//       <div className="mx-auto w-full mt-25 px-2 py-10 flex flex-col md:flex-row gap-8">
...
//                     className={`flex items-start py-1 px-3 rounded-lg transition-all duration-200 ${activeSection === `service-${service.id || service.service_id || idx}`
//                       ? "bg-blue-50 text-blue-600 border-l-4 border-blue-600 "
//                       : "text-gray-600 hover:text-blue-600 hover:bg-gray-50"
//                       }`}
//                   >
...
//                       {idx + 2}.
//                     </span>
//                     <span className="text-sm ">{service.service_name || service.name}</span>
//                   </a>
//                 </li>
//               ))
//             ) : (
...
//         {/* Desktop QuickForm & Payment */}
//         <div className="fixed mt-1 right-20 w-64 space-y-4">
//           <QuickForm />
  ...
  return (
    <div className="min-h-screen ">
      <div className="mx-auto w-full px-4 sm:px-6 lg:px-8 mt-20 sm:mt-25 py-6 sm:py-10 flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-8">
        ...
        {!isMobile && (
          <nav className={`flex-none ${isTablet ? 'lg:w-64' : 'xl:w-80'} sticky top-24 self-start bg-white ml-15 p-4 sm:p-6 h-auto overflow-y-auto `}>
                ...
                services.map((service, idx) => (
                  <li key={service.id || service.service_id || idx}>
                    <a
                      href={`#service-${service.id || service.service_id || idx}`}
                      onClick={() => handleClick(service)}
                      className={`flex items-start py-1 px-2 rounded-lg transition-all duration-200 ${activeSection === `service-${service.id || service.service_id || idx}`
                        ? "bg-blue-50 text-blue-600 border-l-4 border-blue-600"
                        : "text-gray-600 hover:text-blue-600 hover:bg-gray-50"
                        }`}
                    >
        ...
        {/* Main Content */}
        <main className={`flex-1 space-y-6 sm:space-y-8 lg:space-y-10 ${!isMobile ? 'lg:mr-2 xl:mr-2' : ''}`}>
          {/* Overview Section */}
          <section id="menu-overview" className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm scroll-mt-24 sm:scroll-mt-30">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-serif text-center mb-2 text-gray-900">
              {menuData.name || "Startup"}
            </h1>
            <div
          ...
          {services.map((service, idx) => (
            <section
              key={service.id || service.service_id || idx}
              id={`service-${service.id || service.service_id || idx}`}
              className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm scroll-mt-24 sm:scroll-mt-30"
            >
          ...
          {/* FAQ Section */}
          <section id="faq-section" className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-sm scroll-mt-24 sm:scroll-mt-30">
        ...
        {!isMobile && (
          <div className={`flex-none ${isTablet ? 'lg:w-60' : 'xl:w-72'} space-y-4 `}>
            <div className="sticky top-24 space-y-4 mr-15">
              ...
              {/* Payment Section */}
              <div className="bg-white shadow-sm rounded-xl  p-3  w-full border border-gray-100">
                ...
                <div className="mb-2">
                  ...
                  <select
                    ...
                    value={selectedService}
                    onChange={(e) => setSelectedService(e.target.value)}
                    className="w-full border border-gray-300 px-1  py-1  rounded-lg text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition text-sm sm:text-base"
                  >
                    <option value="">-- Choose a service --</option>
                    {services.map((service) => (
                      <option key={service.id || service.service_id} value={service.id || service.service_id}>
                ...
                <button
                  onClick={handleProceed}
                  disabled={!selectedService}
                  className={`w-full font-semibold py-1  rounded-lg transition duration-200 text-sm sm:text-base ${selectedService
                    ? "bg-blue-600 text-white hover:bg-blue-700"
                    : "bg-gray-300 text-gray-600 cursor-not-allowed"
                    }`}
                >
