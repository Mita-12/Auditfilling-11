...
//   const [Menus,setMenus] =useState();// for scroll shadow effect
  ...
  useEffect(() => {
    async function fetchMenus() {
      try {
        ...
        console.log("Fetched menus:", data); // Check API response
        if (Array.isArray(data)) setMenus(data);
        else if (data.menus) setMenus(data.menus);
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <header className="bg-white shadow-md transition-all duration-300">
        <div className="container  ml-55 items-center  px-25 md:px-30 py-4">
          {/* Desktop Menu */}
          <nav className="hidden md:flex items-center gap-10 text-lg z-50">
            {menus.map((menu) => (
              <div
                key={menu.id}
                className="relative"
                onMouseEnter={() => setDropdownOpen(menu.slug)}
                onMouseLeave={() => setDropdownOpen(null)}
              >
                <Link to={menu.url || "#"}>
                  <button className="font-serif tracking-wide text-lg flex items-center gap-1">
                    {menu.name}
                    {menu.submenu?.length > 0 && (
                      <FaChevronDown
                        size={12}
                        className={`transition-transform duration-200 ${
                          dropdownOpen === menu.slug ? "rotate-180" : "rotate-0"
                        }`}
                      />
                    )}
                  </button>
                ...
                {menu.submenu?.length > 0 && (
                  <div
                    className={`absolute left-0 mt-2 bg-white text-gray-900 rounded shadow-lg p-4 z-50 text-sm font-semibold transition-all duration-300
                    ${
                      dropdownOpen === menu.slug
                  ...
                  >
                    {menu.submenu.map((sub) => (
                      <ul key={sub.id} className="space-y-2">
                        {sub.items.map((item) => (
                          <li key={item.id}>
                            <Link
                              to={item.url}
              ...
              {menus.map((menu) => (
                <li key={menu.id}>
                  <button
                    onClick={() =>
                      setMobileSubmenu(
                        mobileSubmenu === menu.slug ? null : menu.slug
                      )
                    }
                    className="flex justify-between w-full font-semibold"
                  >
                    ...
                    {menu.submenu?.length > 0 && (
                      <FaChevronDown
                        className={`transition-transform duration-200 ${
                          mobileSubmenu === menu.slug ? "rotate-180" : ""
                        }`}
                      />
                    )}
                  </button>
                  ...
                  {/* Mobile Submenu */}
                  {mobileSubmenu === menu.slug && menu.submenu?.length > 0 && (
                    <ul className="pl-4 mt-2 flex flex-col gap-2 text-sm">
                      {menu.submenu.map((sub) =>
                        sub.items.map((item) => (
                          <li key={item.id}>
