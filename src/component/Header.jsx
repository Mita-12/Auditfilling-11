...
function Header() {
  ...
  // Fetch menus with services + sub_services
  useEffect(() => {
    async function fetchMenus() {
      try {
        ...
        if (Array.isArray(data)) {
          // normalize menu structure: map services → children
          const formatted = data.map((menu) => ({
            ...menu,
            children: menu.services?.map((service) => ({
              ...service,
              children: service.sub_services || [], // map subservices also
            })),
          }));
          setMenus(formatted);
        } else if (data.menus) {
  ...
  return (
    ...
    >
      ...
      {/* Main Header */}
      <header className="shadow-md bg-white transition-all duration-300">
        <div className="container items-center px-4 md:px-10">
          {/* Desktop Menu */}
          <nav className="hidden md:flex items-center gap-8 text-lg z-50 px-8 py-4">
            {menus.map((menu) => (
              <div
                key={menu.id}
                className="relative"
              ...
              >
                <Link to={menu.slug || "#"}>
                  <button
                    onClick={() =>
                      setDropdownOpen(
                        dropdownOpen === menu.name ? null : menu.name
                      )
                    }
                ...
                {menu.children?.length > 0 && (
                  <div
                    className={`absolute left-0 mt-4 min-w-[300px] bg-white text-gray-900 rounded shadow-sm p-4 grid grid-cols-2 gap-4 z-50 text-[15px] font-semibold transform transition-all duration-300 ${
                      dropdownOpen === menu.name
                        ? "opacity-100 scale-100"
                        : "opacity-0 scale-95 pointer-events-none"
                    }`}
                  >
                    {menu.children.map((service) => (
                      <div key={service.id}>
                        <a
                          href={service.slug || "#"}
                        ...
                        {service.children?.length > 0 && (
                          <ul className="pl-3 mt-1 text-sm text-gray-700 space-y-1">
            ...
            <ul className="flex flex-col gap-3">
              {menus.map((menu) => (
                <li key={menu.id}>
                  ...
                  {mobileSubmenu === menu.name && menu.children?.length > 0 && (
                    <ul className="pl-4 mt-2 flex flex-col gap-2 text-sm">
                      {menu.children.map((service) => (
                        <li key={service.id}>
                          <a
                          ...
                          {service.children?.length > 0 && (
                            <ul className="pl-4 mt-1 flex flex-col gap-1 text-xs text-gray-600">
                              {service.children.map((sub) => (
                                <li key={sub.id}>
                                  <a
                                    href={sub.slug || "#"}
